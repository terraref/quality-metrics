...
def calculate_canopycover(pxarray):
    ...
    gr_img = (g - r) > 0
    gb_img = (g - b) > 0
    mask = gr_img.astype('int')*gb_img.astype('int')
    c = np.count_nonzero(mask)
    ratio = c/float(b.size)

    return ratio, mask

fileName = 'rgb_geotiff_L1_ua-mac_2017-02-19__12-55-28-265_left.tif' # this will ne changed during the computing pipeline
...
try:
    ...
    cc,mask = calculate_canopycover(img)
    logging.info('Estimated canopy cover %f' % (100*cc))
    print('Estimated canopy cover %0.2f' % (100*cc))
